...
char* uci_get(char *file, char *config, char *option)
{
	...
	uci = ucix_init(file, 0);
	value = ucix_get_option(uci, file, config, option);
	ucix_cleanup(uci);

	return value;	/***	Remember to free "value" when using uci_get function*/
}
...
char* uci_get_t(char *file, char *config, char *option)
{
	...
	uci = ucix_init_path(UCI_RUNTIMEDIR,file,0);
	value = ucix_get_option(uci, file, config, option);
	ucix_cleanup(uci);
...
int uci_set_func(char *file,  char *config, char *option, char  *val)
{
        ...
        if(file == NULL){
                printf(" no file name before uci create \n");
                return 0;
        }
        ...
        if(uci)
        {
                ...
                ucix_add_option(uci, file, config, option, val);
                ucix_save(uci, file);
                ucix_cleanup(uci);
...
int uci_set_int_func(char *file,  char *config, char *option, int val)
{
	...
	{	
		...
		ucix_add_option_int(uci, file, config, option, val);

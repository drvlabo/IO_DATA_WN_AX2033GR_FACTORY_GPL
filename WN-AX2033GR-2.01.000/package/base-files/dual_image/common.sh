...
mstc_do_upgrade() {
	...
	if [ "$fw_up_rslt" != 0 ]; then
		...
		/sbin/mstc_persist write bootnum $nextBootNum
		echo "### Switch to bank $nextBootNum on next boot! ###" > /dev/console
	fi

	# delete new firmware info
	/sbin/uci show system | /bin/grep "system.firmware.url" > /dev/null && /sbin/uci delete system.firmware.url && /sbin/uci commit

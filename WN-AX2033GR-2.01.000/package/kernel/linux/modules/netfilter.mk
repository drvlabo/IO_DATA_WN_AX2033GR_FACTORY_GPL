...
define KernelPackage/ipt-siteblock
  TITLE:=Module for block user-space packet
  KCONFIG:=$(KCONFIG_IPT_SITEBLOCK)
  FILES:=$(foreach mod,$(IPT_SITEBLOCK-m),$(LINUX_DIR)/net/$(mod).ko)
  $(call AddDepends/ipt)
  DEPENDS+=+kmod-sbdev
endef

define KernelPackage/ipt-siteblock/description
 Netfilter (IPv4) module for block user-space packet
 Includes:
 - siteblock
endef

$(eval $(call KernelPackage,ipt-siteblock))
...
define KernelPackage/ebtables-siteblock
  TITLE:=ebtables: Siteblock support
  FILES:=$(foreach mod,$(EBTABLES_SITEBLOCK-m),$(LINUX_DIR)/net/$(mod).ko)
  KCONFIG:=$(KCONFIG_EBTABLES_SITEBLOCK)
  $(call AddDepends/ebtables)
  DEPENDS+=+kmod-sbdev
endef

define KernelPackage/ebtables-siteblock/description
 This option adds the siteblock support to ebtables, which siteblock packet
endef

$(eval $(call KernelPackage,ebtables-siteblock))

define KernelPackage/ebtables-time
  TITLE:=ebtables: time match support
  FILES:=$(foreach mod,$(EBTABLES_TIME-m),$(LINUX_DIR)/net/$(mod).ko)
  KCONFIG:=$(KCONFIG_EBTABLES_TIME)
  AUTOLOAD:=$(call AutoProbe,$(notdir $(EBTABLES_TIME-m)))
  $(call AddDepends/ebtables)
endef

define KernelPackage/ebtables-time/description
 This option adds the time match support to ebtables, which allow package in day(time) range
endef

$(eval $(call KernelPackage,ebtables-time))

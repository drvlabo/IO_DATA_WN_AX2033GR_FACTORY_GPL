...
int mtdtype = 0;
/* __MSTC__, Ben add for combo image checksum */
int g_useChecksumInFile = 0;
uint32_t g_hdrchecksum = 0, g_datachecksum = 0;
...
/* To read one or multiple block data from NAND flash. 
    If nextaddr is not NULL, the address of the next block of the last reading block will return */
int mstcnand_read_block(int fd, char *outbuf, unsigned int from, int datalen, unsigned int *nextaddr)
{
	uint32_t rbyte, rtotal = 0;
	ssize_t offset = from;
	int ret = -1;
	char *rbuf = NULL;
	
	/* start address and size must be block alignment */
	if ( ((from%erasesize) != 0) || ((datalen%erasesize) != 0) ) {
		goto quit;
	}
	
	rbuf = malloc(erasesize);
	if (!rbuf) {
		goto quit;
	}
	
	lseek(fd, offset, SEEK_SET);
	while (rtotal < datalen) {
		...
		rbyte = read(fd, rbuf, erasesize);
		if (rbyte == erasesize) {
			memcpy(outbuf+rtotal, rbuf, rbyte);
			rtotal += rbyte;
			offset += rbyte;
		}
		else if (rbyte < 0) {
...
#define BLOCK_ADDR(x)						( x & (~(erasesize - 1)) )

/* To read data from NAND flash. Starting address and size can be not block alignment */
int mstcnand_read(int fd, char *outbuf, uint32_t from, int datalen, uint32_t *nextaddr)
{
	uint32_t readAddr = from;
	uint32_t rsize = 0, remain = datalen;
	uint32_t rtotal = 0;
	int ret = -1;
	
	char *rbuf = malloc(erasesize);	
	if (!rbuf) {
		goto quit_mstcnand_read;
	}	
	
	/* handle beginning non-block-alignment data */
	if ((from%erasesize) != 0) {
		rsize = (datalen <= erasesize) ? datalen : erasesize - (from%erasesize);
		if (mstcnand_read_block(fd, rbuf, BLOCK_ADDR(from), erasesize, &readAddr) < 0) {
			goto quit_mstcnand_read;
		}
		memcpy(outbuf, rbuf+(from%erasesize), rsize);
		remain -= rsize;
		rtotal += rsize;
	}
	
	/* handle block-alignment data */
	if (remain >= erasesize) {
		rsize = remain - (remain%erasesize);
		if (mstcnand_read_block(fd, outbuf+rtotal, readAddr, rsize, &readAddr) < 0) {
			goto quit_mstcnand_read;
		}
		remain -= rsize;
		rtotal += rsize;
	...
	/* handle remaining data (smaller than one block) */
	if (remain) {
		if (mstcnand_read_block(fd, rbuf, readAddr, erasesize, &readAddr) < 0) {
			goto quit_mstcnand_read;
		}
		memcpy(outbuf+rtotal, rbuf, remain);
		//remain -= remain;
		//rtotal += remain;
	}
...
uint32_t bufChecksum(char *data, uint32_t len)
{
	return crc32( 0xffffffffL, data, len) ^  0xffffffffL;
}


int flashChecksum(int fd, unsigned int from, int datalen, uint32_t *checksum)
{
	unsigned int remain = datalen;
	unsigned int readAddr = from;
	unsigned int rsize = 0;
	unsigned long crc = 0xffffffffL;
	int ret = -1;
	char *tmpBuf = malloc(erasesize);
	...
	/* handle beginning non-block-alignment data */
	if ((from%erasesize) != 0) {
		rsize = erasesize - (from%erasesize);
		if (mstcnand_read(fd, tmpBuf, readAddr, rsize, &readAddr) < 0) {
			goto leave;
		}
		crc = crc32(crc, (char *)tmpBuf, rsize);
		remain -= rsize;
	}

	while (remain) {
		rsize = (remain > erasesize) ? erasesize : remain;
		if (mstcnand_read(fd, tmpBuf, readAddr, rsize, &readAddr) < 0) {
			goto leave;
		}
		remain -= rsize;
...
static int
readChecksumFromFile(int fd)
{
	int rbyte;
	image_header_t hdr;
	
	lseek(fd, 0, SEEK_SET);
	rbyte = read(fd, &hdr, sizeof(hdr));
	lseek(fd, 0, SEEK_SET);
	if (rbyte < sizeof(hdr)) {
		return -1;
	}
	g_hdrchecksum = ntohl(hdr.ih_hcrc);
	g_datachecksum = ntohl(hdr.ih_combo_dcrc);
	return 0;
}

static int
mtdFwCheck(const char *mtd)
{
	int fd, ret = -1;
	image_header_t hdr;
	uint32_t len;
	uint32_t nextaddr = 0;
	uint32_t checksum = 0, calChecksum = 0;
	
	fd = mtd_check_open(mtd);
	if(fd < 0) {
		fprintf(stderr, "Could not open mtd device: %s\n", mtd);
		goto quit_mtdFwCheck;
	}

	/* read header */
	if (mstcnand_read(fd, (char*)&hdr, 0, sizeof(hdr), &nextaddr) < 0) {
		fprintf(stderr, "Read flash failed\n");
		goto quit_mtdFwCheck;
	}
	
	/* check magic number */
	if ( ntohl(hdr.ih_magic) != IH_COMBO_MAGIC ) {
		fprintf(stderr, "Invalid magic number\n");
		goto quit_mtdFwCheck;
	}
	
	/* check header checksum */
	checksum = (g_useChecksumInFile) ? g_hdrchecksum : ntohl(hdr.ih_hcrc);
	hdr.ih_hcrc = 0;
	if ( checksum != bufChecksum((char*)&hdr, sizeof(hdr)) ) {
		fprintf(stderr, "Header checksum incorrect\n");
		goto quit_mtdFwCheck;
	}
	printf("Header checksum OK.\n");

	/* check data checksum */
	checksum =  (g_useChecksumInFile) ? g_datachecksum : ntohl(hdr.ih_combo_dcrc);
	len = ntohl(hdr.ih_combo_size);
	if ( (flashChecksum(fd, sizeof(hdr), len, &calChecksum) < 0) || (checksum != calChecksum) ) {
		fprintf(stderr, "Data checksum incorrect\n");
		goto quit_mtdFwCheck;
	}
...
static void usage(void)
{
	...
	/* __MSTC__, Ben to add support combo image checksum */
	"        mstcverify              verify integrity of firmware on device\n"
	"        mstcwrite <imagefile>   write <imagefile> to device\n"
...
int main (int argc, char **argv)
{
	...
	/* __MSTC__, Ben to add support combo image checksum */
	} else if ((strcmp(argv[0], "mstcwrite") == 0) && (argc == 3)) {
		cmd = CMD_MSTCWRITE;
		device = argv[2];
		...
		/* read checksum */
		g_useChecksumInFile = 1;
		if (readChecksumFromFile(imagefd) < 0) {
			fprintf(stderr, "Image check failed.\n");
			exit(1);
		}
	} else if ((strcmp(argv[0], "mstcverify") == 0) && (argc == 2)) {
		cmd = CMD_MSTCVERIFY;
		device = argv[1];
		if (!mtd_check(device)) {
			fprintf(stderr, "Can't open device for writing!\n");
	...
	switch (cmd) {
		...
		case CMD_MSTCWRITE:
			if (!unlocked)
				mtd_unlock(device);
			if (mtd_write(imagefd, device, fis_layout, part_offset) < 0){
				exit(1);
			}
			if (mtdFwCheck(device) < 0) {
				exit(1);
			}
			break;
		case CMD_MSTCVERIFY: /* compare with header on flash */
			if (mtdFwCheck(device) < 0) {
				exit(1);
			}
			break;

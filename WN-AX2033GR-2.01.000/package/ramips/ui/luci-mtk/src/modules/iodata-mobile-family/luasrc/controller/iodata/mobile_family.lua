...
module ("luci.controller.iodata.mobile_family", package.seeall)
...
function index()
	...
	if exanime_mode ~= "repeater" then
		entry({"mobile", "mobile_family"}, call("action_time_limit"), _("Time limit"), 1)
	end
end
...
function action_time_limit()
	...
	luci.template.render("iodata/mobile_family")

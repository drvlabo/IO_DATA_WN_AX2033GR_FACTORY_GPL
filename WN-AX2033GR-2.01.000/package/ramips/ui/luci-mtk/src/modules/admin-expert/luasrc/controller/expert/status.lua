...
function action_session_check()
	luci.template.render("session_check")
end
	...
	if pwwarning_disable == "1" then
		...
		if apply then
			...
			if set_pw == 0 then
				...
				luci.template.render("passWarning",{pw_confirm = -1})
			end
		elseif skip then
			action_status()
		else
...
function action_status()
	...
	--Leo, this will trigger /tmp/tmp_config/runtime.wan_status.connection_time recalculated
	sys.exec("echo nw_get_uptime > /usr/bin/commandpipe")

...
function authenticator.htmlauth(validator, accs, default)
	...
	--MSTC MBA Sean, support for block login ==>
	local block = luci.http.formvalue("block")
	local chk_pass = luci.http.formvalue("login_error")
	
	local user_pass_count=sys.exec("cat /tmp/login_info | wc -l | tr -d '\n'")
	
	if  block or ( user and user_pass_count and tonumber(user_pass_count) >= 5)then --MSTC Sean, block the login 
	...
	require("luci.template")
	context.path = {}
	--[[ MSTC MBA SW Sean, porting from MWRT trunk, start with the webpage at /tmp/web 
	     because we need to customize the htm in the luasrc/view,
	     but we dont want to touch the original base module htm. ]]--

	luci.template.render("sysauth", {duser=default, fuser=user, chk_password = chk_pass})
	
	--luci.template.render("../../../../../tmp/web/sysauth", {duser=default, fuser=user})
...
function dispatch(request)
	...
	if type(target) == "function" then
		...
		if not root or not root.target then
			...
			--MSTC MBA Sean, do not show input path in the error message, to avoid XSS attack

...
module("luci.controller.iodata.mobile_internet", package.seeall)
...
function index()
	...
	if f ~= nil and test_mode == "ap" then
		...
		entry({"mobile", "mobile_internet"}, call("action_internet"), _("Internet Settings"), 2)
	end
end
...
function config_repeater_wireless_ifce()
	...
	-- enable repeater client interface
...
function action_internet()
	...
	if apply then	
		...
		if not wrong_value then
			...
			if (current_mode ~= "router" and set_mode == "router") then
				...
				uci:commit("upnpd")
			elseif (current_mode == "router" and set_mode ~= "router") then
			...
			if ( current_mode ~= set_mode ) then
				...
				sys.exec("/usr/sbin/httpredirect.sh update")
					...
					--MSTC MBA Sean, do not use original mstcIpV6Pass, because it will cause kernel crash
	...
	luci.template.render("iodata/mobile_internet")
end

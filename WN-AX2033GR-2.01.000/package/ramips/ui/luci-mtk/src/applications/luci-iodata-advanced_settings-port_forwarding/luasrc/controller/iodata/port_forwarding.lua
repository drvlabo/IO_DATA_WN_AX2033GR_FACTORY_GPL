...
module ("luci.controller.iodata.port_forwarding", package.seeall)
...
function index()
	...
	if exanime_mode == "router" then
		entry({"content", "advanced_settings", "port_forwarding"}, call("action_port_forwarding"), _("Open Ports"), 1).leaf = true
	end
end

function action_port_forwarding()
	...
	if isApply then
		...
		if #ruleTab <= 32 then
			...
			sys.exec("conntrack -D")
		end

	end
	
	luci.template.render("iodata/port_forwarding")

...
proto_dhcp_setup() {
	...
	# MSTC MBA Sean, do not run dhcp if we support ioilas and ioilas run done yet
	ifce_automode=`uci get network.wan.ifce_automode`
	if [ ! -f /usr/sbin/ioilas ] || [ -f /tmp/ioilas_done ] || [ "$ifce_automode" != "0x01" ]; then
		local IP_version=$(uci get network.wan.IP_version)
		#echo "~~~Jues dhcp.sh IP_version = $IP_version" > /dev/console
		...
		[ -f /sbin/simple_logrec.sh ] && {
			simple_logrec.sh -w "WAN, start DHCP mode"
		}

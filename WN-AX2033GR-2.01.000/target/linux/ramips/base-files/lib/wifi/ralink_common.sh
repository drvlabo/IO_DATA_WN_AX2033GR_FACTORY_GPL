...
enable_ralink_wifi() {
    ...
    # bring up vifs
    local first_vif_enabled=0
    for vif in $vifs; do
        config_get ifname $vif ifname
        config_get disabled $vif disabled
        config_get radio $device radio
        config_get auto_disassociation $device auto_disassociation
        config_get disassociation_threshold $device disassociation_threshold
        config_get association_threshold $device association_threshold
		
        # here's the tricky part. we need at least 1 vif to trigger
        # the creation of all other vifs.
	if [ "$disabled" != "1" -o $first_vif_enabled == 0 ]; then
		ifconfig $ifname up
	...
	if [ "$disabled" == "1" ]; then
        ...
        #for Week Radio Wave Disconnection
        if [ "$auto_disassociation" == "1" ]; then
        	echo "enable $ifname association_threshold=$association_threshold" >> /tmp/wifi.log
        	echo "enable $ifname disassociation_threshold=$disassociation_threshold" >> /tmp/wifi.log
        	iwpriv $ifname set AssocReqRssiThres=$association_threshold
        	iwpriv $ifname set KickStaRssiLow=$disassociation_threshold
		else 
        	echo "disable $ifname association_threshold & disassociation_threshold" >> /tmp/wifi.log
        	iwpriv $ifname set AssocReqRssiThres=0
        	iwpriv $ifname set KickStaRssiLow=0	
        fi

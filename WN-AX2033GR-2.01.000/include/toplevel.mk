...
clean_build_dir:
	@if [ -d build_dir ]; then \
		read -p "Remove build_dir for module checksum(Y/N)N? " useCleanBuildDir ;\
		useCleanBuildDir=$${useCleanBuildDir:-N} ; \
		if [ "$$useCleanBuildDir" = "Y" ] || [ "$$useCleanBuildDir" = "y" ]; then \
			rm -rf build_dir; \
		fi ; \
	fi;

modulechecksum: clean_build_dir source_prepare FORCE
	$(TOPDIR)/moduleChksum_automation_for_fw_releaser.sh
